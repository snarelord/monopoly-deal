...
export default function DiscardModal({
  ...
  const handleDiscard = () => {
    ...
    if (selectedCards.length !== cardsToDiscard) {
      alert(
        `You must discard exactly ${cardsToDiscard} cards to have 7 cards in your hand.`
      );
      return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <p className="mb-4">
          You have {player.hand.length} cards in your hand. You must discard{" "}
          {player.hand.length - 7} cards to end your turn.
        </p>
          ...
          {player.hand.map((card, index) => (
            <div
              ...
              className={`relative transition-transform ${
                selectedCards.includes(index) ? "transform -translate-y-4" : ""
              }`}
