...
export default function PropertySet({ propertySet, onClick }: PropertySetProps) {
  // Get color class based on property color for the border
  const getColorClass = (color: string): string => {
    switch (color) {
      ...
      case "black":
        return "border-gray-800" // For stations
      case "light green":
        return "border-emerald-300" // For utilities
      default:
  ...
  return (
    <div className={`p-2 rounded-lg border-2 ${isComplete ? "border-yellow-400" : colorClass}`} onClick={onClick}>
